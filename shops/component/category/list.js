...
var categoryUrl = commonUrl + "?task=category&request=category&special=1";

function order_checkout() {
	var total = $("#total_selected_sum").html();
	var min_price = parseFloat($("#min_price").html());
	if (total < min_price) {
		alert("The minimum order value is " + min_price + " €");
		return false;
	}
	var checkout_url = commonUrl + "?task=order&request=order&special=3";
...
function add_topping_modal(obj, pid, sid, name) {
	var checkout_url = commonUrl
			+ "?task=category&request=add_topping&special=3&pid="
			+ pid + "&sid=" + sid;
	$('#osx-modal-data').load(checkout_url);
...
viewClass.prototype = {
	init : function() {
		...
		if (op == "category")
			that.select_category(cat_id);
		else if (op == "offer")
			that.select_offer(cat_id);
	...
	order_data_restore : function() {
		...
		that.sendRequest(categoryUrl + insertExtraUrl);
	},
	...
	clickProduct : function(pid, sid) {
		insertExtraUrl = "&product_id=" + pid + "&size_id=" + sid
				+ "&option=selectProduct";
		progressControl(true);
		that.sendRequest(categoryUrl + insertExtraUrl);
	},
	addTopping : function(pid, sid, topping_ids) {
		if (topping_ids == undefined)
			topping_ids = "";
		insertExtraUrl = "&product_id=" + pid + "&size_id=" + sid
				+ "&topping_ids=" + topping_ids + "&option=addTopping";
		progressControl(true);
		that.sendRequest(categoryUrl + insertExtraUrl);
	},
	addSpecial_offer : function(offer_id, offers_str, table_id) {
		if (table_id == undefined)
			table_id = "";
		insertExtraUrl = "&offer_id=" + offer_id + "&offers_str=" + offers_str
				+ "&table_id=" + table_id + "&option=addSpecial_offer";
		progressControl(true);
		that.sendRequest(categoryUrl + insertExtraUrl);
	},
	remove_product : function(obj, pid, sid) {
		var count = parseInt($('#product_' + pid + '_' + sid + ' .count')
				.html());
		count--;
		if (count == 0) {
			...
			$('#product_' + pid + '_' + sid + ' .count').text(count);
		insertExtraUrl = "&product_id=" + pid + "&size_id=" + sid
	...
	calcOrderSum : function() {
		var elms = document.getElementById("order_list").getElementsByTagName(
				"td");
	...
	callAfter : function(p) {
		...
		if (p.sub_exists("product")) {
			...
			if (count == 1) {
				var html = '<table class="selected_product wide" style="display:none" id="'
						+ table_id + '">';
				html += '<tr><td class="thin"></td>';
				html += '<td class="select_arrow"></td>';
				html += '<td class="w66" colspan="3"><span class="product_title">'
						+ name + '</span></td>';
				html += '<td class="product_prce">' + price + '</td>';
				html += '<td class="thin">€</td>';
				html += '</tr>';
				html += '<tr><td></td>';
				html += '<td style="float:center;"><a href="javascript:void(0)" class="add" onclick="that.clickProduct('
						+ id + ',' + size_id + ')"></a>';
				html += '<a href="javascript:void(0)" class="remove" onclick="that.remove_product(this,'
						+ id + ',' + size_id + ')"></a></td>';
				html += '<td class="count">' + count + '</td>';
				html += '<td>';
				if (size)
					html += '<span class="description">' + size + '&nbsp;'
							+ size_detail + '</span>';
				html += '</td>';
				html += '<td >';
				if (topping == 1)
					html += '<a href="javascript:void(0)" class="add_topping" onclick="add_topping_modal(this,'
							+ id
							+ ','
							+ size_id
							+ ',\''
							+ name
							+ '\')" style="width:10px;">Add Topping</a>';
				html += '</td>';
		...
		if (p.sub_exists("toppinglist")) {
			...
			var table_id = 'product_' + id + '_' + size_id;
			$('#product_' + id + '_' + size_id + ' .topping_data').fadeOut(
					"fast");
			$('#product_' + id + '_' + size_id + ' .topping_data').html(
					toppinglist);
			$('#product_' + id + '_' + size_id + ' .topping_data').fadeIn(
					"fast");
