...
class categoryControl
{
	...
	function responseSelectCategory()
	{
		...
		{
			if (!$category[$i]->id_size)
				$category[$i]->id_size = -1;
			if (count($category) == 1)
			{
		...
		for ($i = 0; $i < count($productList); $i++)
		{
			...
			$price = 0;	$arr_tmp = array();
			for ($j = 0; $j < count($category); $j++)
			{
				$arr_tmp[$j] = $priceList[$category[$j]->id_size];
			}
			asort($arr_tmp);
			foreach ($arr_tmp as $j => $price_tmp)
			{
	...
	function responseSessionOrderdata()
	{
		...
		{
			$data = explode(";", $result->order_data);
		}
		for ($i = count($data) - 1; $i >= 0; $i--)
		{
			...
			else if ($row[0] == 2)
			{
				...
				$xml_result .= '<td>';
				if ($product->size)
					$xml_result .= '<span class="description">' . $product->size . '&nbsp;' . $product->size_detail . '</span>';
				$xml_result .= '</td>';
				$xml_result .= '<td>';
				if ($topping)
					$xml_result .= '<a href="javascript:void(0)" class="add_topping" onclick="add_topping_modal(this,' . $row[1] . ',' . $row[2] . ',\'' . $product->name . '\')" style="width:10px;">Add Topping</a>';
				$xml_result .= '</td>';
				$xml_result .= '<td colspan="2"></td>';
				$xml_result .= '</tr>';
	...
	function sessionDataStore($pid, $sid, $topping_ids = "")
	{
		...
		elseif ($result->order_data)
		{
			$order_data = ($result->order_data) . ";2," . $pid . "," . $sid . ",1,";
			$count = 1;
		}
		else
		{
			$order_data = "2," . $pid . "," . $sid . ",1,";
			$count = 1;
		}
	...
	function sessionOfferDataStore($offer_id, $offers_str = "")
	{
		...
		elseif ($result)
		{
			$order_data = ($result->order_data) . ";1," . $offer_id . "," . $offers_str . ",1";
			$count = 1;
		}
		else
		{
			$order_data = "1," . $offer_id . "," . $offers_str . ",1";
			$count = 1;
		}
		$row = new session_table($database);
		$row->id = $result->id;
	...
	function responseRemoveProduct()
	{
		...
		for ($i = 0; $i < count($data); $i++)
		{
			$row = explode(",", $data[$i]);
			//if($row[0]!=2) continue;
			if ($row[0] == 2 && $this->product_id == $row[1] && $this->size_id == $row[2])
				$row[3]--;
			$count = $row[3];
			if ($count)
				array_push($_data, implode(",", $row));
